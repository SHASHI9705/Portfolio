...
const Project6Card: React.FC = () => (
  <>
    <img src="/images/shellacount.png" alt="Account Creation Script" className="w-full h-80 object-cover rounded-xl mb-4 border border-white/30 shadow-md mt-0" />
    <div className="w-full flex flex-col items-start justify-start pl-2">
      <h3 className="text-2xl md:text-3xl font-semibold text-left mb-2 tracking-wide bg-gradient-to-r from-blue-400 via-green-400 to-blue-400 bg-[length:200%_200%] bg-clip-text text-transparent animate-gradient-x flex items-center">
        ...
        <a
          href="https://github.com/SHASHI9705/User-Account-Creation"
          target="_blank"
        ...
        <a
          href="https://www.linkedin.com/posts/shashiranjan-singh9_shellscripting-linux-useraccount-activity-7280904493016219648-dOvp?utm_source=social_share_send&utm_medium=member_desktop_web&rcm=ACoAAFXiSDUBMRX_NEvjsKl4GEvCMLDb8TxRyho"
          target="_blank"
          rel="noopener noreferrer"
          className="ml-2 flex items-center justify-center w-10 h-10 rounded-full bg-blue-500/30 border border-blue-400/40 hover:bg-yellow-400/30 transition-colors duration-200 shadow transform hover:scale-110"
          style={{marginBottom: '-2px'}}
      ...
      <p className="text-sm md:text-base text-gray-100 text-left leading-relaxed font-normal">User Account Creation Script is a Bash-based automation tool that quickly creates Linux user accounts. It ensures root access, generates time-based passwords, and enforces password reset on first login — making user setup fast and secure.</p>
      <div className="mt-4 flex flex-wrap gap-2">
        <div className="inline-block px-5 py-1.5 bg-black text-blue-100 font-semibold text-sm rounded-full shadow select-none">Linux</div>
        <div className="inline-block px-5 py-1.5 bg-black text-blue-100 font-semibold text-sm rounded-full shadow select-none">Shell Script</div>
        <div className="inline-block px-5 py-1.5 bg-black text-blue-100 font-semibold text-sm rounded-full shadow select-none">User</div>
        <div className="inline-block px-5 py-1.5 bg-black text-blue-100 font-semibold text-sm rounded-full shadow select-none">Security</div>
      </div>
    </div>
  </>
);
